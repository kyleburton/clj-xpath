...
This section is under development, if you have a particular situation you'd like have added as an example, please write to me at "kyle.burton@gmail.com":mailto:kyle.burton@gmail.com

h3. Get all the Child Nodes of a given Element

h3. Find a node with a specific attribute value

...show xpath queries using attribute matchers eg: "//*[@foo='bar']"

h3. Finding a node that contains specific text

...show xpath query using content matcher: "//*[contains(content(@foo),"some text")]"

h3. Find nodes with one of several tag names

...show example of using 'or'

LINK TO AN XPATH DOCUMENTATION SITE

h2. Building

clj-xpath is not in a maven repository at this time.  clj-xpath is built with maven at this time.  It requires both clojure (1.0) and clojure-contrib (1.0) to be installed in an accessible maven repository.  The software includes a script to install them into your local (<code>~/.m2</code>) repository for you
...
* get clj-xpath into a maven repository (like clojars)
